...
//댓글 시작
	...
	$(document).on('click','.modify-btn',function(){
		...
		var modifyUI = '<form id="mnews_comment_form">';
		   ...
		   modifyUI += '  <textarea rows="3" cols="50" name="comm_con" id="mcomm_con" class="rep-content">' + content + '</textarea>';
		   ...
		   modifyUI += '</form>';
			...
			//입력한 글자수 셋팅
			var inputLength = $('#mcomm_con').val().length;
			var remain = 300 - inputLength;
			remain += '/300';
		...
		//댓글 수정
		$(document).on('submit','#mnews_comment_form',function(event){
			...
			$.ajax({
				...
				success:function(param){
					if(param.result=='logout'){
						...
						$('#mnews_comment_form').parent().find('p').html($('#mcomm_con').val().replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>'));
						//수정폼 삭제 및 초기화
						initModifyForm();
